<section>
	<h1 class="center">Results - <?= $Model[$test_id]["title"] ?></h1>
	<div class="score">
		<h2>You scored <?= $score ?> in <?= $time_taken ?> minute(s).</h2>
		<p class="score-text"></p>
	</div>
	<h2 class="percentage center"><?= $score/$total*100 ?>%</h2>
...
<?php	
	if (count($subjects) > 1) {
		...
		foreach ($subjects as $key => $subject) {
			$total_questions = 0;
			$scored = calculate_score($questions, [$subject], $user_answers);
